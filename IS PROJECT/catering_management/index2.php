...
if (!isset($_SESSION['deliname'])) {
    header("Location: index.html");
}else{
  $filter = $_SESSION['deliname'];
...
<head>
  ...
  <title>Catering Management System - Delivery Personnel Homepage</title>
  <!-- slider stylesheet -->
...
$sql = " SELECT `order_id`, `order_total`, `no_of_people`, `payment_method`, `order_status`, `Location`, `Custom` FROM `tbl_order` WHERE `order_status` = 'Accepted' OR `delivery_id` = '$filter'";
...
<?php
  ...
  ?>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure that you want to transport this order?')?window.location.href='insertion.inc.php?action=updateO&id=<?php echo($row["order_id"]); ?>&id1=<?php echo($row["order_status"]); ?>':true;" title='Transport Order'>Transport</button></td>
<?php
}else if ($row["order_status"] == "On Route") {
  ?>
<td><button class="btn btn-primary py-3 px-5" onclick="return confirm('Are you sure that you want to deliver this order?')?window.location.href='insertion.inc.php?action=updateO&id=<?php echo($row["order_id"]); ?>&id1=<?php echo($row["order_status"]); ?>':true;" title='Deliver Order'>Deliver</button></td>
<?php
        ...
        <div class="container" id="contact">
          ...
          <div class="form_container">
              <button name="" onclick="genRep();" type="submit">
                Generate Report of Delivery and Customers
              </button>
          </div>
        </div>
      <h2>
